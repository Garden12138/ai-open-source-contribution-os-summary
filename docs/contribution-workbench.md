...
入口“制定贡献方案”现在打开对话与可编辑方案。AI 从贡献任务的不可变分析和
...
启用规划／执行需要 `IMPLEMENTATION_PROVIDER=nvidia_nim`、
`REVIEW_PROVIDER=nvidia_nim`、`SANDBOX_STAGE_RUNTIME=docker`、有效的
`SANDBOX_JOB_SPEC_SIGNING_KEY` 和 `WORKBENCH_RUNNER_IMAGE=sha256:<digest>`。
规划使用实现模型，审查使用独立的 `REVIEW_MODEL` 调用。
